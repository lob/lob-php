...
use OpenAPI\Client\Model\UsVerification;
use OpenAPI\Client\Model\UsVerificationOrError;
use OpenAPI\Client\Model\UsVerifications;
...
/**
 * USVerificationsApiTest Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
...
class USVerificationsApiUnitTest extends TestCase
{
    /**
     * Setup before running any test cases
     */
    private static $mockVerificationId;
    private static $mockVerificationsWritable;
    private static $mockUsVerificationResponse;

    private static $mockMultipleComponentsList;
    private static $mockUsVerificationBulkResponse;

    // set up constant fixtures
    public function setUp(): void
    {
        self::$mockVerificationId = "us_ver_fakeId";
        self::$mockVerificationsWritable = new UsVerificationsWritable();
        self::$mockUsVerificationResponse = new UsVerification();
        self::$mockUsVerificationResponse->setId(self::$mockVerificationId);

        // Bulk Fixture
        $item1 = new MultipleComponents();
        $item2 = new MultipleComponents();
        $item3 = new MultipleComponents();
        $item1->setPrimaryLine("123 street");
        $item2->setPrimaryLine("456 street");
        $item3->setPrimaryLine("789 street");

        self::$mockMultipleComponentsList = new MultipleComponentsList();
        self::$mockMultipleComponentsList->setAddresses([$item1, $item2, $item3]);
        $resp1 = new UsVerificationOrError();
        $resp2 = new UsVerificationOrError();
        $resp3 = new UsVerificationOrError();
        $resp1->setPrimaryLine("123 street");
        $resp2->setPrimaryLine("456 street");
        $resp3->setPrimaryLine("789 street");
        self::$mockUsVerificationBulkResponse = new UsVerifications();
        self::$mockUsVerificationBulkResponse->setAddresses(array ($resp1, $resp2, $resp3));
    ...
    public function testBulkConnectionError()
    {
        ...
        try {
            ...
            $unhappyPath = $verificationsApi->verifySingle(self::$mockVerificationsWritable);
    ...
    public function testSingle()
    {
        ...
        try {
            $happyPath = $verificationsApi->verifySingle(self::$mockVerificationsWritable);
    ...
    public function testSingleApiError()
    {
        ...
        try {
            $this->expectException(ApiException::class);
            $this->expectExceptionMessageMatches("/blah/");
            $errorResponse = $verificationsApi->verifySingle(self::$mockVerificationsWritable);
        } catch (Exception $deleteError) {
            echo 'Caught exception: ', $deleteError->getMessage(), "\n";
        }
    }
