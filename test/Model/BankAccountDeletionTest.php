...
class BankAccountDeletionTest extends TestCase
{
    ...
    {
        // Procedural
        $address = new BankAccountDeletion();
        $address->setId("bank_fake");
        $this->assertEquals($address->getId(), "bank_fake");

        // Construction
        $address = new BankAccountDeletion(["id" => "bank_fake2"]);
        $this->assertEquals($address->getId(), "bank_fake2");

        // Array Access
        $address = new BankAccountDeletion();
        $address["id"] = "bank_fake3";
        $this->assertEquals($address->getId(), "bank_fake3");
    }
    ...
    public function testPropertyObject()
    {
        // Procedural
        $address = new BankAccountDeletion();
        $address->setObject("bank_account_deleted");
        $this->assertEquals($address->getObject(), "bank_account_deleted");

        // Construction
        $address = new BankAccountDeletion(["object" => "bank_account_deleted"]);
        $this->assertEquals($address->getObject(), "bank_account_deleted");

        // Array Access
        $address = new BankAccountDeletion();
        $address["object"] = "bank_account_deleted";
        $this->assertEquals($address->getObject(), "bank_account_deleted");
    }
    ...
    {
        $address = new BankAccountDeletion();
        $this->assertEquals($address->getModelName(), "bank_account_deletion");
    }
    ...
    {
        $list = new BankAccountDeletion();
        $list->offsetSet('object', 'bank_account_deleted');
        $this->assertEquals($list->offsetExists('object'), true);
        $this->assertEquals($list->offsetGet('object'), 'bank_account_deleted');
